...
'the same format used for FPGA timestamps
  ...
  Dim rewrite
  Dim old_ver3, new_ver3, old_ver1, old_ver2, whole_new_revision
  Dim yymmdd, hrmn, rev1_2, rev3_00
...
'call MsgBox ("hrmn_from_FormatDate(FormatDateTime(Time,4)) looks like this: " & hrmn ) 
  ...
  if i > 0 then
	...
	new_ver3 = (1+old_ver3)
    copy_of_contents = Mid(contents_of_file,1,n) _
       & Right("000" & new_ver3,3) _
       & "\0" _
     & Mid(contents_of_file,k)
    contents_of_file = copy_of_contents
  end if

  'Parse out 3 pieces of version info for FPGA-format timestamp/revision
  ' Save this for when we do the new "FPGA" format implementation below.
  ...
  rev3_00 = ascii_hex16_from_2_twoDigit_ascii_decimals(new_ver3,"00") 

  'Save new revision for completion mession at end of script
  whole_new_revision = "V" & old_ver1 & "." & old_ver2 & "." & new_ver3
...
'call MsgBox ("rev3_00 looks like this: " & rev3_00 ) 
...
'See if Timestamp.c file exists.
  ...
  'Examine all .vhd files in the folder
  Set fso = CreateObject("Scripting.FileSystemObject")
