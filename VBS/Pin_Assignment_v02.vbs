...
'
'v01 - works exclusively for adding a single 'SIGNAL=<net-name>;<pin#>' attribute,
'      and it assumes the component identifier record, for ex: 'I 2215 Connectors:C62HDFVDSUB 1 50 900 0 1 '
'      is followed immediately by a 'A 84 910 10 0 3 3 REFDES=P?' attribute type record.
'      This is NOT a generalized approach, but it worked well enough the first time.
'
'v02 - drop the requirement to have a REFDES attribute record immediately following the 'I' record.
'      Add an option to either add a 'SIGNAL=' attribute or a "TABLE=" attribute.
'- - - - - - - - - - - - - - -
'
...
'- - - - - - - - - - - - - - -
'
Function Get_Table_Number(Selected_Component)
'
'Ask user to enter TB# for TABLE= attributes
...
'
  ...
  '- - - - - Get Signal - - - - -
  prompt_string = "Enter the table number " & Chr(13) & Chr(10)_
                & "for the ""TABLE=TB#"" attribute"  & Chr(13) & Chr(10)_
                & " " & Chr(13) & Chr(10)_
                & "(looking for a number here,"  & Chr(13) & Chr(10)_
                & "like ""2"", not ""TB2"")"   & Chr(13) & Chr(10)_
                & " " 

  prompt_string = fix_prompt_string(prompt_string) 'to work better w/ InputBox
  Table_Num = "2"
  Table_Num = InputBox(prompt_string, "Pin_Assignment",_
                 Table_Num)
  'If operator clicks CANCEL, InputBox returns zero length string ("")
  if (Table_Num = "") Then WScript.Quit 'operator CANCELED script
 ...
 '- - - - - Verification - - - - -
  prompt_string = "Next step is to create the edited file. " & Chr(13) & Chr(10)_
				...
				& Chr(13) & Chr(10)_
				& "   " & "TABLE=TB" & TABLE_Num & Chr(13) & Chr(10)_
				& Chr(13) & Chr(10)_
  ...
  Get_Table_Number = Table_Num
End Function
'- - - - - - - - - - - - - - -
'
Function Get_Option_for_Signal_or_Table()
' Ask if they want us to add 'SIGNAL=xxx;nn' type attributes, 
' or 'TABLE=TBn' type attributes.
'
  Dim Prompt_String
  Dim ok_or_cancel
  Dim I
  ...
  '- - - - - Get Signal - - - - -
  prompt_string = "Enter a number to select your option: " & Chr(13) & Chr(10)_
                & Chr(13) & Chr(10)_
                & "1 -- add ""SIGNAL=xxx;nn"" type attributes"  & Chr(13) & Chr(10)_
                & Chr(13) & Chr(10)_
                & "2 -- add ""TABLE=TBn"" type attributes"  & Chr(13) & Chr(10)_
                & " " & Chr(13) & Chr(10)_
                & " " 

  prompt_string = fix_prompt_string(prompt_string) 'to work better w/ InputBox
  Option_Num = "1"
  Option_Num = InputBox(prompt_string, "Pin_Assignment",_
                 Option_Num)
  'If operator clicks CANCEL, InputBox returns zero length string ("")
  if (Option_Num = "") Then WScript.Quit 'operator CANCELED script
  Get_Option_for_Signal_or_Table = Option_Num
End Function
'- - - - - - - - - - - - - - -
'
...
'- - - - - - - - - - - - - - -
'
Sub  Read_File_Write_Edited_File(schematic_file_path, input_schematic_file_name, Selected_Component, _
                                 Signal, Signal_or_Table_Option, Table_Number)
'This is where we finally edit the target file.
'Read a line at a time and write to the destination file making changes as we go along
'So far we either (based on value of Signal_or_Table_Option)
'1) Add "SIGNAL=<net name>;<pin#>" Attributes, or
'2) Add	"TABLE=TB<X>" attributes
'							 
  Dim f_read, Line, fso, f_write, Add_Line, Temp_Line
  Dim Val, token, I, token_5, token_6
  Const ForReading = 1, ForWriting = 2, ForAppending = 8
  ...
  Do While (f_read.AtEndOfStream <> True)  
    ...
    Do 'actually, we aren't looping, really, just a creative
	  ...
	  If(LCase(token) = Selected_Component) Then
		...
		'Found our component record, lets get 2 tokens out of it, for use in new records
	    Temp_Line = Line
        token_5 = get_space_separated_token(Temp_Line,5) 'third token from Line
	    Temp_Line = Line
        token_6 = get_space_separated_token(Temp_Line,6) 'third token from Line
		
		If (Signal_or_Table_Option = 1) Then
		  'Add a SIGNAL= attribute
		  Line = "A 50 " & (token_6-10) & " 10 0 3 0 SIGNAL=" & Signal
		Else
		  'Add a TABLE=TBx attribute
		  Line = "A " & (token_5+30) & " " & (token_6+6) & " 10 0 4 3 TABLE=TB" & Table_Number
		End If
...
'
  ...
  Signal_or_Table_Option = Get_Option_for_Signal_or_Table()
  ...
  If (Signal_or_Table_Option = 1) Then
     ...
     Table_Number = Get_Table_Number(Selected_Component) 'function call
  End If
  
  call Read_File_Write_Edited_File(schematic_file_path, input_schematic_file_name, Selected_Component, _
                                   Signal_and_Pin_Number, Signal_or_Table_Option, Table_Number)
